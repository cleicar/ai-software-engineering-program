...
function makeContext(products, users) {
    ...
    // Computar a media de idade dos comporadores por produto
...
async function trainModel({ users }) {
    ...
    context.productVectors = products.map(product => {
        return {
            name: product.name,
            meta: { ...product },
            vector: encodeProduct(product, context).dataSync() // dataSync() converts the tensor to a plain JavaScript array
        }
    })

    context.usersVectors = users.map(user => {
        return {
            name: user.name,
            meta: { ...user },
            vector: encodeUser(user, context).dataSync() // convert to array of floats
        }
    })
